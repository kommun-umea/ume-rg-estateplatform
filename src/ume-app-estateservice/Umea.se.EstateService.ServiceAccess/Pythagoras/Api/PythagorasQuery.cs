...
public class PythagorasQuery<T> where T : class
{
    ...
    {
        string name = ExpressionHelpers.GetApiName(selector);
        string str = ExpressionHelpers.FormatValue(value);
    ...
    {
        string name = ExpressionHelpers.GetApiName(s);
        Filter minFilter = new(target, name, Op.Ge, ExpressionHelpers.FormatValue(min));
        Filter maxFilter = new(target, name, Op.Le, ExpressionHelpers.FormatValue(max));
    ...
    {
        QueryRequest newReq = _req with { OrderBy = new Order(ExpressionHelpers.GetApiName(s), true) };
        return new PythagorasQuery<T>(newReq, _usedSkip, _usedTake, _usedPage);
    }

    public PythagorasQuery<T> OrderByDescending<TProp>(Expression<Func<T, TProp>> s)
    {
        QueryRequest newReq = _req with { OrderBy = new Order(ExpressionHelpers.GetApiName(s), false) };
        return new PythagorasQuery<T>(newReq, _usedSkip, _usedTake, _usedPage);
    }
